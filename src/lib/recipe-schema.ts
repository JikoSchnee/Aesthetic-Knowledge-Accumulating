...
export function isCurrentRecipe(value: unknown): value is Record<string, unknown> {
  ...
  return Boolean(metadata && typeof metadata.title === "string" && typeof metadata.category === "string" && strings(metadata.retrievalTags) && profile && typeof profile.description === "string" && strings(profile.triggerTerms) && strings(profile.excludeWhen) && typeof item.visualDefinition === "string" && strings(item.coreVisualRelationships, 3) && strings(item.coreInvariants) && typeof item.reuseFormula === "string" && isTypography(item.typographyAndGraphicLanguage));
}
