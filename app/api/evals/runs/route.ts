...
export async function POST(request: Request) {
  try {
    const body = await request.json().catch(() => ({})) as { caseIds?: string[]; topK?: number; library?: LibraryType | "all"; concurrency?: number; groupName?: string; name?: string; embedding?: EmbeddingConfig };
