    ...
    for (let index = 0; index < recipes.length; index += 1) {
      ...
      try {
        if (stored.typographySchemaVersion === TYPOGRAPHY_SCHEMA_VERSION && isTypography(stored.recipe?.typographyAndGraphicLanguage)) skipped += 1;
          ...
          stored.recipe.typographyAndGraphicLanguage = typography; stored.recipeSchemaVersion = "1.1"; stored.typographySchemaVersion = TYPOGRAPHY_SCHEMA_VERSION; stored.typographyStatus = "ready"; stored.typographyModel = vision.model; stored.typographyUpdatedAt = new Date().toISOString(); delete stored.typographyError;
          ...
          if (search) { const array = (value: unknown) => Array.isArray(value) ? value : []; search.typographyText = typographySearchText; search.searchText = [search.title, search.category, ...array(search.medium), ...array(search.useCases), ...array(search.tags), ...array(search.coreRelationships), search.reuseFormula, typographySearchText].filter(Boolean).join(" · "); search.typographyStatus = "ready"; search.typographySchemaVersion = TYPOGRAPHY_SCHEMA_VERSION; search.searchSchemaVersion = "1.1"; }
          if (hasEmbeddingConfig(embedding)) {
            try { const generated = await createSkillEmbedding(stored.id, stored.recipe, embedding); await writeFile(join(embeddingsDir, `${stored.id}.json`), JSON.stringify(generated, null, 2)); stored.embeddingStatus = "ready"; stored.embeddingModel = embedding.model; stored.embeddingUpdatedAt = generated.createdAt; stored.indexStatus = "hybrid_searchable"; if (search) { search.embeddingStatus = "ready"; search.embeddingModel = embedding.model; search.embeddingUpdatedAt = generated.createdAt; } vectorsRebuilt += 1; }
