...
type EvalRun = { id: string; createdAt: string; updatedAt: string; groupName?: string; status: "running" | "paused" | "completed"; config: GenerationSettings & { topK: number; library: Library; concurrency?: number }; cases: CaseRun[]; progress: { completed: number; failed: number; total: number; percent: number } };
...
export function EvalStudio({ embedding, onOpenSettings }: { embedding: EmbeddingConfig; onOpenSettings: () => void }) {
  ...
  const deleteGroup = async (name: string) => { if (!window.confirm(`删除历史组“${name}”及其中所有运行和生成结果？此操作不会删除 Eval 原图。`)) return; const response = await fetch(`/api/evals/runs?group=${encodeURIComponent(name)}`, { method: "DELETE" }); const result = await response.json() as { deletedIds?: string[]; error?: string }; if (!response.ok) { setMessage(result.error || "删除失败。"); return; } const deleted = new Set(result.deletedIds || []); setRuns((current) => { const next = current.filter((item) => !deleted.has(item.id)); setActiveRun((active) => active && deleted.has(active.id) ? next[0] : active); return next; }); setMessage(`已删除历史组“${name}”。`); };
