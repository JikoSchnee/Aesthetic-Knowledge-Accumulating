...
export function EvalStudio({ embedding, onOpenSettings }: { embedding: EmbeddingConfig; onOpenSettings: () => void }) {
  ...
  const create = async () => {
    ...
    try { await saveGenerationSelection(); const response = await fetch("/api/evals/runs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ caseIds: [...selected], topK, library, concurrency: generationConcurrency, groupName, name: runName, embedding }) }); const result = await response.json() as EvalRun & { error?: string }; if (!response.ok) throw new Error(result.error || "无法创建运行。"); setActiveRun(result); setRuns((current) => [result, ...current]); setMessage(`运行已创建并冻结 ${result.snapshot?.candidateCount || 0} 个候选，最多 ${result.config.concurrency || 3} 个 Case 并行推进。`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "无法创建运行。"); }
    finally { setCreating(false); }
  };
  const action = async (nextAction: "pause" | "resume") => { if (!activeRun) return; const response = await fetch(`/api/evals/runs/${activeRun.id}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: nextAction, embedding }) }); const result = await response.json() as EvalRun & { error?: string }; if (!response.ok) { setMessage(result.error || "无法更新运行。"); return; } setActiveRun(result); setRuns((current) => current.map((item) => item.id === result.id ? result : item)); if (result.pauseReason) setMessage(result.pauseReason); };
  const renameGroup = async (currentName: string) => { const nextName = window.prompt("输入新的历史组名", currentName); if (!nextName?.trim() || nextName.trim() === currentName) return; const response = await fetch("/api/evals/runs", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ groupName: currentName, nextGroupName: nextName }) }); const result = await response.json() as EvalRun[] & { error?: string }; if (!response.ok) { setMessage(result.error || "重命名失败。"); return; } setRuns(result); setActiveRun((current) => current ? result.find((item) => item.id === current.id) : result[0]); };
  const renameRun = async (run: EvalRun) => { const nextName = window.prompt("编辑任务名称", run.name || `Eval ${new Date(run.createdAt).toLocaleString()}`); if (!nextName?.trim()) return; const response = await fetch(`/api/evals/runs/${run.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: nextName }) }); const result = await response.json() as EvalRun & { error?: string }; if (!response.ok) { setMessage(result.error || "编辑失败。"); return; } setRuns((current) => current.map((item) => item.id === result.id ? result : item)); setActiveRun((current) => current?.id === result.id ? result : current); };
  const deleteRun = async (run: EvalRun) => { if (!window.confirm(`删除该次任务及其本地运行记录、生成结果？此操作不会删除 Eval 原图。`)) return; const response = await fetch(`/api/evals/runs/${run.id}`, { method: "DELETE" }); const result = await response.json() as { error?: string }; if (!response.ok) { setMessage(result.error || "删除失败。"); return; } setRuns((current) => { const next = current.filter((item) => item.id !== run.id); setActiveRun((active) => active?.id === run.id ? next[0] : active); return next; }); setMessage(`已删除任务“${run.id.slice(0, 12)}”。`); };
  ...
  return <div className="page eval-page">
    <section className="eval-control">
      <div className="eval-intro"><span className="eyebrow">FIXED IMAGE SET / RESUMABLE RUNS</span><h2>让同一批图片，反复检验你的 Skill。</h2><p>原图只进入临时分析；Top K 中每个匹配 Skill 都会单独生成一张结果，并保存检索依据与运行记录，不污染正式审美库。</p></div>
      <div className="eval-config-grid">
        ...
        <label>生图并发<select value={generationConcurrency} onChange={(event) => setGenerationConcurrency(Number(event.target.value))}>{Array.from({ length: 8 }, (_, index) => index + 1).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
        <label>历史组名<input value={groupName} maxLength={80} onChange={(event) => setGroupName(event.target.value)} placeholder="例如：杂志封面 v1"/></label>
        <label>任务名称<input value={runName} maxLength={80} onChange={(event) => setRunName(event.target.value)} placeholder="例如：8 月产品图测试"/></label>
      </div>
