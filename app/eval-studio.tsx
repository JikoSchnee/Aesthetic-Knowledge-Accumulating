...
export function EvalStudio({ embedding, onOpenSettings }: { embedding: EmbeddingConfig; onOpenSettings: () => void }) {
  ...
  return <div className="page eval-page">
    <section className="eval-control">
      <div className="eval-intro"><span className="eyebrow">FIXED IMAGE SET / RESUMABLE RUNS</span><h2>让同一批图片，反复检验你的 Skill。</h2><p>原图只进入临时分析；系统会从 Top K 匹配 Skill 中随机选取指定数量，分别生成结果，并保存检索依据与运行记录，不污染正式审美库。</p></div>
      <div className="eval-config-grid">
        ...
        <label>Top K 候选数<div className="eval-range"><input type="range" min="1" max="10" value={topK} onChange={(event) => { const value = Number(event.target.value); setTopK(value); setRandomPickCount((current) => Math.min(current, value)); }}/><b>{topK}</b></div></label>
        <label>随机选取数（最多 {topK}）<input type="number" inputMode="numeric" min="1" max={topK} step="1" value={randomPickCount} onChange={(event) => setRandomPickCount(Math.max(1, Math.min(topK, Math.round(Number(event.target.value) || 1))))}/></label>
