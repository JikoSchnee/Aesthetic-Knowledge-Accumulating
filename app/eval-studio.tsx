...
type CaseRun = { caseId: string; filename: string; stage: string; matches?: Match[]; generations?: GenerationRun[]; prompt?: string; resultFile?: string; error?: string; retryCount?: number; nextRetryAt?: string; lastTransientError?: string; remoteId?: string; remoteState?: string; timings: Record<string, number> };
type EvalRun = { id: string; createdAt: string; updatedAt: string; groupName?: string; name?: string; status: "running" | "paused" | "completed"; config: GenerationSettings & { topK: number; library: Library; concurrency?: number }; cases: CaseRun[]; progress: { completed: number; failed: number; total: number; percent: number } };
