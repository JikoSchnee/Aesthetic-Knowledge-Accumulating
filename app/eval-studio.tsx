...
const defaultGeneration: GenerationSettings = { provider: "openrouter", model: "google/gemini-2.5-flash-image", endpoint: "https://openrouter.ai/api/v1", apiKey: "", outputFormat: "png", falInputTemplate: "", falResultPath: "images.0.url" };
  ...
  return <div className="page eval-page">
    <section className="eval-control">
      ...
      <div className="eval-config-grid">
        ...
        <label>模型 / Endpoint ID{generation.provider === "fal" && generation.presets?.length ? <select value={generation.model} onChange={(event) => setGeneration((current) => ({ ...current, model: event.target.value }))}>{generation.presets.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select> : <input value={generation.model} onChange={(event) => setGeneration((current) => ({ ...current, model: event.target.value }))}/>}</label>
...
export function ImageGenerationSettingsPanel() {
  ...
  return <section className="embedding-sheet generation-settings"><div className="embedding-head"><div><span className="eyebrow">IMAGE GENERATION</span><h3>Eval 生图 Provider</h3><p>密钥只保存在服务端；fal.ai 使用可恢复 Queue 任务。</p></div><div className="embedding-count"><b>{values.provider === "fal" ? "FAL" : "OR"}</b><small>{values.apiKey ? "KEY READY" : "NO KEY"}</small></div></div><div className="settings-sheet embedding-fields"><div className="settings-field"><label>Provider</label><select value={values.provider} onChange={(event) => providerChange(event.target.value as "openrouter" | "fal")}><option value="openrouter">OpenRouter Images</option><option value="fal">fal.ai Queue</option></select></div><div className="settings-field"><label>模型 / Endpoint ID</label><input value={values.model} onChange={(event) => setValues((current) => ({ ...current, model: event.target.value }))}/></div><div className="settings-field full"><label>API Endpoint</label><input value={values.endpoint} onChange={(event) => setValues((current) => ({ ...current, endpoint: event.target.value }))}/></div><div className="settings-field"><label>API Key</label><div className="key-input"><input type="password" value={values.apiKey} placeholder={values.apiKey === "env-configured" ? "已保存在 .env；更换时粘贴新 Key" : "粘贴生图 API Key"} onChange={(event) => setValues((current) => ({ ...current, apiKey: event.target.value }))}/><span>{values.apiKey ? "● 已配置" : "○ 未输入"}</span></div></div><div className="settings-field"><label>输出格式</label><select value={values.outputFormat} onChange={(event) => setValues((current) => ({ ...current, outputFormat: event.target.value as GenerationSettings["outputFormat"] }))}><option value="png">PNG</option><option value="jpeg">JPEG</option><option value="webp">WebP</option></select></div>{values.provider === "fal" && <><div className="settings-field full"><label>自定义输入 JSON 模板</label><textarea value={values.falInputTemplate} onChange={(event) => setValues((current) => ({ ...current, falInputTemplate: event.target.value }))}/><small>允许：{`{{prompt}} · {{image_url}} · {{image_urls}}`}</small></div><div className="settings-field"><label>结果图片 JSON 路径</label><input value={values.falResultPath} onChange={(event) => setValues((current) => ({ ...current, falResultPath: event.target.value }))}/></div></>}<footer><small>{message || "OpenRouter 使用同步 Images API；fal.ai 保存 request_id 后轮询。"}</small><button className="ink-btn" onClick={save} disabled={saving}>{saving ? "正在保存…" : "保存生图配置"}</button></footer></div></section>;
}
